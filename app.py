...
st.sidebar.markdown("---")
st.sidebar.header("🏦 투자자별 매매동향")
trend_weeks = st.sidebar.selectbox(
    "외국인·기관 순매매 조회 기간", [1, 2, 3, 4, 8, 12], index=0,
    format_func=lambda w: f"최근 {w}주",
    help="선택한 기간 동안의 외국인·기관 일자별 순매매 거래량을 표시합니다. "
         "4주 초과 기간은 네이버 여러 페이지를 이어붙여 조회합니다. (국내 주식 한정)"
)
...
# 공용 헬퍼: 재시도 기능이 있는 HTTP GET → BeautifulSoup (네트워크 일시 오류 대응)
def _fetch_soup(url, tries=3, timeout=5):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    for i in range(tries):
        try:
            res = requests.get(url, headers=headers, timeout=timeout)
            res.raise_for_status()
            res.encoding = res.apparent_encoding  # 네이버 금융은 EUC-KR
...
# 헬퍼 함수 2: 네이버 금융 크롤링 (재시도 + 캐싱으로 안정성 확보)
...
# 헬퍼 함수 3: 네이버 금융 투자자별 매매동향(외국인·기관 순매매) 크롤링
def _parse_signed_cell(td):
    """순매매량 셀 파싱. 명시적 '-' 또는 파란색(순매도) 클래스면 음수로 반환."""
    span = td.find('span')
    raw = (span.get_text(strip=True) if span else td.get_text(strip=True))
    cls = ' '.join(span.get('class', [])) if span else ''
    mag = re.sub(r'[^0-9]', '', raw)
    if mag == '':
        return 0
    val = int(mag)
    if ('-' in raw) or ('nv' in cls) or ('down' in cls) or ('blue' in cls):
        val = -val
    return val

def _parse_investor_rows(soup):
    """frgn 페이지 soup에서 '날짜(YYYY.MM.DD) + 9개 열' 일자별 매매동향 행만 추출."""
    rows = []
    for tr in soup.find_all('tr'):
        ...
        if not re.match(r'^\d{4}\.\d{2}\.\d{2}$', date_txt):
            continue
        inst = _parse_signed_cell(tds[5])   # 기관 순매매량
        frgn = _parse_signed_cell(tds[6])   # 외국인 순매매량
        rows.append({'날짜': date_txt, '기관 순매매량': inst, '외국인 순매매량': frgn})
    return rows

@st.cache_data(ttl=1800)
def get_naver_investor_trend(code, pages=1):
    """네이버 금융 frgn 페이지를 pages 개수만큼 크롤링해 일자별 외국인·기관 순매매를 반환.
    페이지당 약 20거래일을 제공하므로 긴 기간은 여러 페이지를 이어붙인다.
    반환: ['날짜','기관 순매매량','외국인 순매매량'] DataFrame (순매수 +, 순매도 -)."""
    all_rows = []
    for p in range(1, pages + 1):
        ...
        if soup is None:
            break
        rows = _parse_investor_rows(soup)
        if not rows:
    ...
    df = pd.DataFrame(all_rows).drop_duplicates(subset=['날짜']).reset_index(drop=True)
    return df

# 헬퍼 함수 4: yfinance 시세 조회 (캐싱 + 재시도로 Yahoo 요청 제한 대응)
@st.cache_data(ttl=900, show_spinner=False)
def get_price_history(ticker, start, end):
    """단일 종목 일봉. 빈 결과는 KS/KQ 판별·상장폐지 신호로 그대로 반환한다."""
    for i in range(3):
        try:
...
def get_ticker_info(ticker):
    """yfinance .info (ROE/ROIC 등). 실패 시 빈 dict 반환하여 렌더링 중단을 방지한다."""
    for i in range(3):
        try:
...
if st.sidebar.button("📊 실시간 차트 생성"):
    ...
    if not target_ticker:
        ...
        if final_ticker:
            ...
            try:
                data = get_price_history(final_ticker, start_date, end_date)
                ...
                if data.empty:
                    st.error(
                        "⚠️ 데이터를 불러오지 못했습니다.\n\n"
                        "상장폐지·거래정지 종목이거나, Yahoo Finance의 일시적 요청 제한(rate limit)일 수 있습니다. "
                        "잠시 후 '📊 실시간 차트 생성'을 다시 눌러 주세요."
                    )
                else:
                    ...
                    if compare_index:
                        ...
                        idx_data = get_index_history(benchmark_ticker, start_date, end_date)
                    ...
                    custom_hover_text = [
                        f"<b>{d.strftime('%Y-%m-%d')}</b><br><br>시가: {currency_sym}{o:{p_format}}{currency_unit}<br>종가: {currency_sym}{c:{p_format}}{currency_unit}<br>최고: {currency_sym}{h:{p_format}}{currency_unit}<br>최저: {currency_sym}{l:{p_format}}{currency_unit}"
                        for d, o, h, l, c in zip(data.index, open_price, high_price, low_price, close_price)
                    ]

                    fig.add_trace(go.Candlestick(x=data.index, open=open_price, high=high_price, low=low_price, close=close_price, name="주가", increasing_line_color='red', decreasing_line_color='blue', text=custom_hover_text, hoverinfo="text", hovertemplate="%{text}<extra></extra>", showlegend=False), row=1, col=1, secondary_y=False)
                    ...
                    if indicator_choice == "RSI (과열/침체)":
                        ...
                        # 💡 MACD 서브플롯(row 3) 내부에 전용 범례 박스 추가
                        fig.add_annotation(
                            x=0.01, y=0.95, xref="paper", yref="y4 domain",
                            text="<b><span style='color:black'>■ MACD</span> &nbsp; <span style='color:orange'>■ Signal</span> &nbsp; <span style='color:red'>■ 상승 모멘텀(+)</span> &nbsp; <span style='color:blue'>■ 하락 모멘텀(-)</span></b>",
                            showarrow=False,
                            font=dict(size=12),
                    ...
                    for y in years:
                        ...
                        if not year_data.empty:
                            ...
                            fig.add_annotation(x=mid_d, y=1.02, yref='paper', showarrow=False, text=f"<b>| ◀ {y}년도 ▶ |</b>", font=dict(size=14, color="gray"))
                            if start_d > data.index[0]:
                                fig.add_vline(x=start_d, line_dash="dash", line_color="gray", opacity=0.5)

                    fig.update_layout(
                        title=f"<b>{target_ticker.upper()} ({final_ticker}) 통합 분석</b>",
                    ...
                    # --- [투자자별 매매동향] 주가 차트와 가격 지표 사이에 배치 (한국 주식 한정) ---
                    st.markdown(f"### 🏦 투자자별 매매동향 (최근 {trend_weeks}주)")

                    if is_us_stock:
                        st.info("투자자별 매매동향(외국인·기관·개인)은 국내 주식에만 제공됩니다.")
                    else:
                        # 페이지당 약 20거래일 → 필요 페이지 수를 넉넉히 산정 (버퍼 +1, 최대 6)
                        trend_pages = min(6, math.ceil((trend_weeks * 5) / 18) + 1)
                        trend_df = get_naver_investor_trend(search_code, pages=trend_pages)
                        if trend_df.empty:
                            st.info("투자자별 매매동향 데이터를 불러오지 못했습니다. (네이버 금융 응답 확인 필요)")
                        else:
                            trend_df['날짜_dt'] = pd.to_datetime(trend_df['날짜'], format='%Y.%m.%d')
                            period_days = trend_weeks * 7
                            cutoff = pd.Timestamp(datetime.today().date()) - pd.Timedelta(days=period_days)
                            period_df = trend_df[trend_df['날짜_dt'] >= cutoff].copy()
                            # 휴장 등으로 기간 내 데이터가 없으면 최근 거래일(주당 약 5일)로 대체
                            if period_df.empty:
                                period_df = trend_df.sort_values('날짜_dt', ascending=False).head(trend_weeks * 5).copy()
                            period_df = period_df.sort_values('날짜_dt')

                            # 개인 순매매(추정) = -(외국인 + 기관). 기타법인 순매매까지 흡수하는 근사치.
                            period_df['개인(추정) 순매매량'] = -(period_df['외국인 순매매량'] + period_df['기관 순매매량'])

                            # 네이버 첫 페이지 데이터가 선택 기간보다 짧을 수 있음을 안내
                            oldest = period_df['날짜_dt'].min()
                            if oldest > cutoff:
                                st.caption(f"※ 네이버 제공 범위 내에서 {period_df['날짜'].iloc[0]}부터의 데이터를 표시합니다.")

                            frgn_sum = int(period_df['외국인 순매매량'].sum())
                            inst_sum = int(period_df['기관 순매매량'].sum())
                            indiv_sum = int(period_df['개인(추정) 순매매량'].sum())

                            s_spacer, s_col1, s_col2, s_col3 = st.columns([0.1, 1, 1, 1])
                            with s_col1: st.metric(f"외국인 {trend_weeks}주 순매매", f"{frgn_sum:+,} 주",
                                                    help="양수(+)=순매수, 음수(-)=순매도")
                            with s_col2: st.metric(f"기관 {trend_weeks}주 순매매", f"{inst_sum:+,} 주",
                                                    help="양수(+)=순매수, 음수(-)=순매도")
                            with s_col3: st.metric(f"개인(추정) {trend_weeks}주 순매매", f"{indiv_sum:+,} 주",
                                                    help="개인 = -(외국인+기관) 역산 추정치")

                            chart_col, table_col = st.columns([1.4, 1])

                            with chart_col:
                                x_labels = period_df['날짜_dt'].dt.strftime('%m.%d')
                                bar_fig = go.Figure()
                                bar_fig.add_trace(go.Bar(
                                    x=x_labels, y=period_df['외국인 순매매량'], name='외국인',
                                    marker_color='crimson',
                                    hovertemplate='%{x}<br>외국인 순매매: %{y:+,}주<extra></extra>'
                                ))
                                bar_fig.add_trace(go.Bar(
                                    x=x_labels, y=period_df['기관 순매매량'], name='기관',
                                    marker_color='royalblue',
                                    hovertemplate='%{x}<br>기관 순매매: %{y:+,}주<extra></extra>'
                                ))
                                bar_fig.add_trace(go.Bar(
                                    x=x_labels, y=period_df['개인(추정) 순매매량'], name='개인(추정)',
                                    marker_color='seagreen',
                                    hovertemplate='%{x}<br>개인(추정) 순매매: %{y:+,}주<extra></extra>'
                                ))
                                bar_fig.add_hline(y=0, line_color='gray', line_width=1)
                                bar_fig.update_layout(
                                    barmode='group', template='plotly_white', height=360,
                                    margin=dict(t=30, b=20, l=10, r=10),
                                    yaxis_title='순매매 거래량(주)',
                            ...
                            with table_col:
                                trade_cols = ['외국인 순매매량', '기관 순매매량', '개인(추정) 순매매량']
                                disp = period_df[['날짜'] + trade_cols].copy()

                                def color_signed(v):
                                    color = 'red' if v > 0 else ('blue' if v < 0 else 'black')
                                    return f'color: {color}'

                                styler = disp.style.format({c: '{:+,.0f}' for c in trade_cols})
                                # pandas 2.1+ 에서 Styler.applymap → Styler.map 으로 변경됨 (버전 호환 처리)
                                _apply = styler.map if hasattr(styler, 'map') else styler.applymap
                                styler = _apply(color_signed, subset=trade_cols)
                                st.dataframe(styler, use_container_width=True, hide_index=True)

                            st.caption(f"최근 {trend_weeks}주 · 총 {len(period_df)}거래일 · 개인은 -(외국인+기관) 역산 추정치 · 빨강(+)=순매수, 파랑(-)=순매도 · 단위: 주 · 출처: 네이버 금융")
                    ...
                    if is_us_stock:
                        info = get_ticker_info(final_ticker)
                        ...
                        roe_display = f"{roe * 100:.2f}%" if roe not in ['N/A', None] else "N/A"
                        roic_display = f"{roic * 100:.2f}%" if roic not in ['N/A', None] and isinstance(roic, (int, float)) else "N/A"
                    else:
                        ...
                        kr_info = get_ticker_info(final_ticker)
                        kr_roe = kr_info.get('returnOnEquity', 'N/A')
                        kr_roic = kr_info.get('returnOnCapital', 'N/A')

                        roe_display = f"{kr_roe * 100:.2f}%" if kr_roe not in ['N/A', None] else "N/A"
                        roic_display = f"{kr_roic * 100:.2f}%" if kr_roic not in ['N/A', None] else "N/A"

                    f_spacer, f_col1, f_col2, f_col3, f_col4, f_col5, f_col6 = st.columns([0.1, 1, 1, 1, 1, 1, 1])
                    with f_col1: st.metric("시가총액", mc_display)
                    with f_col2: st.metric("PER (가치)", pe_display)
                    with f_col3: st.metric("PBR (가치)", pb_display)
                    with f_col4: st.metric("ROE (수익성)", roe_display)
                    with f_col5: st.metric("ROIC (수익성)", roic_display)
                    with f_col6: st.metric("배당수익률", div_display)
