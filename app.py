        ...
        if final_ticker:
            ...
            try:
                ...
                if data.empty:
                    ...
                    # --- [3] 투자자별 매매동향 (사이드바 선택 기간, 한국 주식 한정) ---
                    st.markdown("---")
                    st.markdown(f"### 🏦 투자자별 매매동향 (최근 {trend_weeks}주)")

                    if is_us_stock:
                        st.info("투자자별 매매동향(외국인·기관 순매매)은 국내 주식에만 제공됩니다.")
                    else:
                        ...
                        if trend_df.empty:
                            ...
                            s_spacer, s_col1, s_col2 = st.columns([0.1, 1, 1])
                            ...
                            with table_col:
                                disp = period_df[['날짜', '외국인 순매매량', '기관 순매매량']].copy()

                                def color_signed(v):
                                    color = 'red' if v > 0 else ('blue' if v < 0 else 'black')
                                    return f'color: {color}'

                                styler = disp.style.format({'외국인 순매매량': '{:+,.0f}', '기관 순매매량': '{:+,.0f}'})
                                # pandas 2.1+ 에서 Styler.applymap → Styler.map 으로 변경됨 (버전 호환 처리)
                                _apply = styler.map if hasattr(styler, 'map') else styler.applymap
                                styler = _apply(color_signed, subset=['외국인 순매매량', '기관 순매매량'])
                                st.dataframe(styler, use_container_width=True, hide_index=True)

                            st.caption(f"최근 {trend_weeks}주 · 총 {len(period_df)}거래일 · 빨강(+)=순매수, 파랑(-)=순매도 · 단위: 주 · 출처: 네이버 금융")
