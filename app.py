...
def get_naver_investor_trend(code):
    """네이버 금융 frgn 페이지에서 일자별 외국인·기관 순매매 거래량을 파싱한다.
    반환: ['날짜','기관 순매매량','외국인 순매매량'] 컬럼의 DataFrame (순매수 +, 순매도 -)."""
    ...
    try:
        def parse_signed(td):
            ...
            if mag == '':
                return 0
            val = int(mag)
            # 부호 판별: 명시적 '-' 또는 파란색(순매도) 클래스면 음수
            if ('-' in raw) or ('nv' in cls) or ('down' in cls) or ('blue' in cls):
                val = -val
            return val

        # 페이지 전체에서 '날짜(YYYY.MM.DD)로 시작하고 9개 열을 가진' 일자별 매매동향 행만 수집
        for tr in soup.find_all('tr'):
            ...
            if not re.match(r'^\d{4}\.\d{2}\.\d{2}$', date_txt):
                continue
            inst = parse_signed(tds[5])   # 기관 순매매량
            frgn = parse_signed(tds[6])   # 외국인 순매매량
        ...
        if final_ticker:
            ...
            try:
                ...
                if data.empty:
                    ...
                    # --- [3] 투자자별 매매동향 (최근 1주일, 한국 주식 한정) ---
                    st.markdown("---")
                    st.markdown("### 🏦 투자자별 매매동향 (최근 1주일)")

                    if is_us_stock:
                        st.info("투자자별 매매동향(외국인·기관 순매매)은 국내 주식에만 제공됩니다.")
                    else:
                        trend_df = get_naver_investor_trend(search_code)
                        if trend_df.empty:
                            st.info("투자자별 매매동향 데이터를 불러오지 못했습니다. (네이버 금융 응답 확인 필요)")
                        else:
                            trend_df['날짜_dt'] = pd.to_datetime(trend_df['날짜'], format='%Y.%m.%d')
                            one_week_ago = pd.Timestamp(datetime.today().date()) - pd.Timedelta(days=7)
                            week_df = trend_df[trend_df['날짜_dt'] >= one_week_ago].copy()
                            # 휴장 등으로 7일 내 데이터가 없으면 최근 5거래일로 대체
                            if week_df.empty:
                                week_df = trend_df.sort_values('날짜_dt', ascending=False).head(5).copy()
                            week_df = week_df.sort_values('날짜_dt')

                            frgn_sum = int(week_df['외국인 순매매량'].sum())
                            inst_sum = int(week_df['기관 순매매량'].sum())

                            s_spacer, s_col1, s_col2 = st.columns([0.1, 1, 1])
                            with s_col1: st.metric("외국인 주간 순매매", f"{frgn_sum:+,} 주",
                                                    help="양수(+)=순매수, 음수(-)=순매도")
                            with s_col2: st.metric("기관 주간 순매매", f"{inst_sum:+,} 주",
                                                    help="양수(+)=순매수, 음수(-)=순매도")

                            chart_col, table_col = st.columns([1.4, 1])

                            with chart_col:
                                x_labels = week_df['날짜_dt'].dt.strftime('%m.%d')
                                bar_fig = go.Figure()
                                bar_fig.add_trace(go.Bar(
                                    x=x_labels, y=week_df['외국인 순매매량'], name='외국인',
                                    marker_color='crimson',
                                    hovertemplate='%{x}<br>외국인 순매매: %{y:+,}주<extra></extra>'
                                ))
                                bar_fig.add_trace(go.Bar(
                                    x=x_labels, y=week_df['기관 순매매량'], name='기관',
                            ...
                            with table_col:
                                disp = week_df[['날짜', '외국인 순매매량', '기관 순매매량']].copy()
                            ...
                            st.caption("빨강(+)=순매수, 파랑(-)=순매도 · 단위: 주 · 출처: 네이버 금융")
