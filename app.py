...
# 💡 핵심 추가: 커스텀 CSS 주입 (제목 및 지표 글자 크기 1/3 축소)
st.markdown("""
<style>
    /* 1. '가격 지표', '기업 펀더멘털 요약' 등 h3 제목 크기 축소 */
    h3 {
        font-size: 1.3rem !important;
        padding-bottom: 0.2rem !important;
    }
    
    /* 2. 지표 위의 작은 라벨(현재가, 최고가, 시가총액 등) 폰트 미세 조정 */
    div[data-testid="stMetricLabel"] > label > div {
        font-size: 0.85rem !important;
    }
    
    /* 3. 실제 금액/수치 데이터 폰트 크기를 기존의 약 2/3 로 대폭 축소 */
...
start_date = st.sidebar.date_input("조회 시작일", value=datetime.today() - timedelta(days=720))
end_date = st.sidebar.date_input("조회 종료일", value=datetime.today())

st.sidebar.markdown("---")
st.sidebar.header("📈 보조지표 설정")
indicator_choice = st.sidebar.radio(
    "하단 차트에 추가할 지표를 선택하세요",
    ["선택 안 함", "RSI (과열/침체)", "MACD (추세강도)"]
)
...
def format_market_cap_us(value):
    ...
    try:
        value = float(value)
        if value >= 1e12:
            return f"${value/1e12:.2f}T (조 달러)"
        elif value >= 1e9:
            return f"${value/1e9:.2f}B (십억 달러)"
        else:
            return f"${value:,.0f}"
    except:
        return "N/A"

# 네이버 금융 크롤링 함수 (한국 주식용)
...
def get_naver_finance_info(code):
    ...
    try:
        ...
        if market_sum:
            info['market_cap'] = market_sum.text.strip() + "억 원"

        per = soup.select_one('#_per')
        if per:
            info['per'] = per.text.strip() + "배"

        pbr = soup.select_one('#_pbr')
        if pbr:
            info['pbr'] = pbr.text.strip() + "배"

        dvr = soup.select_one('#_dvr')
        if dvr:
            info['dividend'] = dvr.text.strip() + "%"
    ...
    if not target_ticker:
        ...
        if final_ticker:
            ...
            try:
                ...
                if data.empty:
                    st.error("데이터를 불러올 수 없습니다. 티커/종목코드나 상장 여부를 확인해 주세요.")
                else:
                    ...
                    with col1:
                        st.metric("현재가", f"{currency_sym}{current_price:{p_format}} {currency_unit}".strip())
                    with col2:
                        st.metric("최고가", f"{currency_sym}{highest_price:{p_format}} {currency_unit}".strip())
                    with col3:
                        st.metric("최저가", f"{currency_sym}{lowest_price:{p_format}} {currency_unit}".strip())
                    with col4:
                    ...
                    with f_col1:
                        st.metric("시가총액", mc_display)
                    with f_col2:
                        st.metric("PER (주가수익비율)", pe_display)
                    with f_col3:
                        st.metric("PBR (주가순자산비율)", pb_display)
                    with f_col4:
                        st.metric("배당수익률", div_display)
